...
*   **For the Test Cases in This Paper:** Topological detection confirms that the coordinates of the three fixed obstacles (`-2`) defined in this paper do not form an enclosed barrier, and the effective grid graph indeed constitutes a single connected component. Therefore, the state space calculation and the conclusion of 100% reachability in the paper are entirely correct.
...
**2. The Underlying Topological and Algebraic Mechanisms for "Parity Constraint Failure"**
Page 6 of the paper states: "The 'internal permutation' characteristic of homogeneous blocks effectively collapses the parity isolation." In the theoretical review after finalization, the author further clarified the **dual underlying mathematical mechanisms** that break the classical parity deadlock of sliding puzzles:
*   **Covert Swaps of Homogeneous Blocks:** Based on the pigeonhole principle, the board inevitably contains a large number of homogeneous blocks (e.g., 9 `1`s and 12 `-1`s). Covert transpositions of homogeneous blocks at the underlying level alter the parity of the microscopic permutation while the macroscopic physical state remains unchanged. In the mapping to the quotient space, this causes a topological collapse of the parity subspaces.
...
Given that homogeneous blocks and wildcard blocks can break parity, under what extreme conditions would the parity isolation of this variant sliding puzzle be restored (i.e., an absolute deadlock appears)?
...
The **sole mathematical condition** for the restoration of parity isolation is: the quantity of every type of movable homogeneous block is strictly $\le 1$ (i.e., losing the ability for covert swaps, and the cursor lacks extra wildcard blocks to act as a parity toggle switch).
...
*   **Theoretical Singularity:** Parity would only lock when a massive number of obstacles confine the cursor to an extreme dead-end where $N_{valid} \le 3$ (e.g., a $1 \times 3$ single chain, losing topological cycles and algebraic surrogates). However, in this case, the state space is extremely small (on the order of $O(1)$), and the algorithm can complete the determination in microseconds, never causing a dimensionality disaster.